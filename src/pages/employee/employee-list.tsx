...
const EmployeeList = () => {
  ...
  mock.forEach((row) => {
    const newData = {
      ...
      role: row.position.position_name,
      status: '0',
      orderStatus: row.employment.employment_name,
      contact: row.phone_number,
      country: row.job_category.job_category_name,
      address: row.address,
        ...
        Cell: ({ row }: { row: Row<{}> }) => {
          return (
            <Stack direction="row" alignItems="center" justifyContent="center" spacing={0}>
              <Tooltip title="表示">
                <IconButton color="secondary" onClick={() => navigation(`/employee/detail/${row.values.id}/basic`)}>
